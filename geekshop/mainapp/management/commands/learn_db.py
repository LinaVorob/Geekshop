...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        action_2__condition = Q(order__updated__lte=F('order__created') + action_2__time_delta) & Q(
            order__updated__gt=F('order__created') + action_1__discount)
        ...
        for orderitem in test_orders:
            print(f'{orderitem.action_order:2}: заказ №{orderitem.pk}:'
